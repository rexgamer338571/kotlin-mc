...
class ListTag<T>(val elementType: Tag.TagType, value: List<T>, name: String = "") : Tag<List<T>>(value, name) {
    companion object {
        fun <T : Tag<Any>> make(value: List<T>, name: String): ListTag<T> =
            ListTag(
                if (value.isEmpty()) TagType.UNDEFINED else
                    NBT.TAG_TYPE_CLASS_MAP.getB(value.first()::class)!!, value, name
            )

        fun <T : Tag<Any>> make(value: List<T>): ListTag<T> = make(value, "")
    }
